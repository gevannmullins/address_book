...
class DatabaseAdapter {
    ...
    public function getQuery($tableName, $select=['*'], $where=[], $sort=[], $limit='' )
    {
        // Stringify the select array
        $select_string = '';
        if ($select[0] == '*') {
            $select_string = '*';
        } elseif (count($select)>1) {
            foreach ($select as $s) {
                $select_string .= $select_string . ' ' . $s;
            }
        }

        // Stringify the where array
        $where_string = '';
        if (count($where)>0) {
            foreach ($where as $keyW=>$valueW) {
                $where_string .= $where_string . ' ' . $w;
            }
        }

        $sql = "SELECT $select_string FROM $tableName WHERE ";
