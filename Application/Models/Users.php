...
class ModelsUsers extends Model {
    ...
    public function userExist($useremail, $userpassword)
    {
        $sql = "SELECT * FROM " . DB_PREFIX . "users WHERE user_email = '" . $useremail . "' AND user_login_pass = '" . $userpassword . "';";
        $query = $this->db->query($sql);
        return ($query->num_rows > 0) ? true : false;
    }

    public function getUserInfo($useremail, $userpassword)
    {
        $sql = "SELECT * FROM " . DB_PREFIX . "users WHERE user_email = '" . $useremail . "' AND user_login_pass = '" . $userpassword . "';";
        $query = $this->db->query($sql);
        return $query->row;
