...
function CheckCampaignStartDate(){
	...
	var get_gld_est1 = $("#est_googledisplay").html();
	var get_gld_est2 = get_gld_est1.replace(/,/g, "");
	var get_gld_est = parseFloat(get_gld_est2);
	var gld_est = Math.round(get_gld_est);
	
	var get_gyt_est1 = $("#est_youtube").html();
	var get_gyt_est2 = get_gyt_est1.replace(/,/g, "");
	var get_gyt_est = parseFloat(get_gyt_est2);
	var gyt_est = Math.round(get_gyt_est);
...
//	}
//	if(gld_est>0){
//		var gldMaxDays = Math.floor(gld_est/1000);
//	}else{
//		var gldMaxDays = 0;
//	}
	if(sms_est>0 && sms_est>1000){
		var smsMaxDays = Math.floor(sms_est/1000);
	}else if(sms_est>0 && sms_est<1000){
		var smsMaxDays = 1;
	}else{
		var smsMaxDays = 0;
	}

	if(email_est>0 && email_est>1000){
		var emailMaxDays = Math.floor(email_est/1000);
	}else if(email_est>0 && email_est<1000){
		var emailMaxDays = 1;
	}else{
		var emailMaxDays = 0;
	}

	if(fb_est>0 && fb_est>1000){
		var fbMaxDays = Math.floor(fb_est/1000);
	}else if(fb_est>0 && fb_est<1000){
		var fbMaxDays = 1;
	}else{
		var fbMaxDays = 0;
	}

	if(gls_est>0 && gls_est>1000){
		var glsMaxDays = Math.floor(gls_est/1000);
	}else if(gls_est>0 && gls_est<1000){
		var glsMaxDays = 1;
	}else{
		var glsMaxDays = 0;
	}

	if(gld_est>0 && gld_est>1000){
		var gldMaxDays = Math.floor(gld_est/1000);
	}else if(gld_est>0 && gld_est<1000){
		var gldMaxDays = 1;
	}else{
		var gldMaxDays = 0;
	}

	if(gyt_est>0 && gyt_est>1000){
		var gytMaxDays = Math.floor(gyt_est/1000);
	}else if(gyt_est>0 && gyt_est<1000){
		var gytMaxDays = 1;
	}else{
		var gytMaxDays = 0;
	}
	
	var MinDiffDays = nonZeroMin(smsMaxDays, emailMaxDays, fbMaxDays, glsMaxDays, gldMaxDays, gytMaxDays);
	if(MinDiffDays>30){MinDiffDays = 30;}
	...
	if((sms_est>0) && (email_est==0) && (fb_est==0) && (gls_est==0) && (gld_est==0) && (gyt_est==0)){
	...
	var EstDailyReach = Math.floor(sms_est/diff1)+Math.floor(email_est/diff1)+Math.floor(fb_est/diff1)+Math.floor(gls_est/diff1)+Math.floor(gld_est/diff1)+Math.floor(gyt_est/diff1);
	$("#daily_est_days").html(EstDailyReach+' est daily reach');
//	if(sms_est>0){
...
function nonZeroMin() { // input can be as many parameters as you want
	...
	var MinDiffDays = nonZeroMin(smsMaxDays, emailMaxDays, fbMaxDays, glsMaxDays, gldMaxDays, gytMaxDays);
	var MaxDiffDays = Math.max(smsMaxDays, emailMaxDays, fbMaxDays, glsMaxDays, gldMaxDays, gytMaxDays);
	if(MinDiffDays>30){MinDiffDays = 30;}
	if(MaxDiffDays>30){MaxDiffDays = 30;}
