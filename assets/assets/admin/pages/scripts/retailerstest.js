...
var Retailers = function () {
    ...
    var form = $('#storeowners-unlinked-name-filter-form');
    ...
    var initUnlinkedRetailers = function () {
        // init linked stores table
        unlinkedGrid = new Datatable();

        unlinkedGrid.init({
            src: $("#storeowners-table"),
            onSuccess: function (unlinkedGrid) {
            },
            onError: function (unlinkedGrid) {
            },
            onDataLoad: function (unlinkedGrid) {
            },
            loadingMessage: 'Loading...',
            dataTable: {
                responsive: true,
                "language": {
                    "emptyTable": "All the retailers have been linked."
                },
                "bServerSide": false,
                "ajax": {
        ...
        // handle group actionsubmit button click
        unlinkedGrid.getTableWrapper().on('click', '.table-group-action', function (e) {
            e.preventDefault();
            var action = $(this);
            if (action.val() != "" && unlinkedGrid.getSelectedRowsCount() > 0) {
                unlinkedGrid.setAjaxParam("customActionType", "group_action");
                unlinkedGrid.setAjaxParam("customActionName", action.val());
                unlinkedGrid.setAjaxParam("id", unlinkedGrid.getSelectedRows());
                unlinkedGrid.getDataTable().ajax.reload(function () {
                    grid.getDataTable().ajax.reload();
                });
                unlinkedGrid.clearAjaxParams();
            } else if (action.val() == "unlink_all" && unlinkedGrid.getRowsCount() > 0) {
                unlinkedGrid.setAjaxParam("customActionType", "group_action");
                unlinkedGrid.setAjaxParam("customActionName", action.val());
                unlinkedGrid.setAjaxParam("id", unlinkedGrid.getAllRows());
                unlinkedGrid.getDataTable().ajax.reload(function () {
                    grid.getDataTable().ajax.reload();
                });
                unlinkedGrid.clearAjaxParams();
                ...
                toastr.error('Please select an action', 'Error');
            } else if (unlinkedGrid.getSelectedRowsCount() === 0 || unlinkedGrid.getRowsCount() === 0) {
    ...
    var reloadUnlinkedStores = function () {
        unlinkedGrid.getDataTable().ajax.reload();
    }
