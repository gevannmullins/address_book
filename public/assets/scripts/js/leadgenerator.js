...
//		$('#OneTimePin').val('');
			...
			$.ajax({
					...
					success: function(data, textStatus, jqXHR){
						...
						if(parseFloat(ReminingCredits)<0){
							$('#payment_selector_next').text('Proforma Invoice');
						}
					},
					error: function (jqXHR, textStatus, errorThrown){
	...
	$("#payment_selector_payfast").click(function(e){
			...
			success: function(data, textStatus, jqXHR){
				if(data>0){
					...
					if(parseFloat(ReminingCredits)>=0){
						...
						$('#new_header').html('<strong>New:</strong> <font style="color:#A1C938;">Proforma Invoice</font>');
						$("#payment_selector").hide("slide");
						$("#purchase_selector").show("slide");
						$('#OneTimePin').val('');
						var ThisLink = $('#geolink').val();
						...
						var CampBudget = getCampBudget.replace(/,/g, "");
						var ReminingCredits = parseFloat(Total) - parseFloat(CampBudget);
						//alert('../includes/call.fn.createpdf.invoice.html.php?id='+data+'');
						$('#payfast').load('../includes/call.fn.createpdf.invoice.html.php?id='+data+'');
						$('html, body').animate({
							scrollTop: $("#new").offset().top
						}, 2000);
					}
